...
class SermondistributorViewHelp_documents extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		// [Interpretation 8464] Set Type Selection
		$this->typeOptions = $this->getTheTypeSelections();
		if ($this->typeOptions)
		{
			// [Interpretation 8468] Type Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8477] Type Batch Selection
				JHtmlBatch_::addListSelection(
		...
		// [Interpretation 8464] Set Location Selection
		$this->locationOptions = $this->getTheLocationSelections();
		if ($this->locationOptions)
		{
			// [Interpretation 8468] Location Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8477] Location Batch Selection
				JHtmlBatch_::addListSelection(
		...
		// [Interpretation 8464] Set Admin View Selection
		$this->admin_viewOptions = $this->getTheAdmin_viewSelections();
		if ($this->admin_viewOptions)
		{
			// [Interpretation 8468] Admin View Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8477] Admin View Batch Selection
				JHtmlBatch_::addListSelection(
		...
		// [Interpretation 8464] Set Site View Selection
		$this->site_viewOptions = $this->getTheSite_viewSelections();
		if ($this->site_viewOptions)
		{
			// [Interpretation 8468] Site View Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8477] Site View Batch Selection
				JHtmlBatch_::addListSelection(
	...
	{
		// [Interpretation 8340] Get a db connection.
		$db = JFactory::getDbo();

		// [Interpretation 8342] Create a new query object.
		$query = $db->getQuery(true);

		// [Interpretation 8344] Select the text.
		$query->select($db->quoteName('type'));
		$query->from($db->quoteName('#__sermondistributor_help_document'));
		$query->order($db->quoteName('type') . ' ASC');

		// [Interpretation 8348] Reset the query using our newly populated query object.
		$db->setQuery($query);
		...
		if ($results)
		{
			// [Interpretation 8356] get model
			$model = $this->getModel();
			$results = array_unique($results);
			$filter = array();
			foreach ($results as $type)
			{
				// [Interpretation 8367] Translate the type selection
				$text = $model->selectionTranslation($type,'type');
				// [Interpretation 8369] Now add the type and its text to the options array
	...
	protected function getTheLocationSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [Interpretation 8367] Translate the location selection
				$text = $model->selectionTranslation($location,'location');
				// [Interpretation 8369] Now add the location and its text to the options array
	...
	protected function getTheAdmin_viewSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [Interpretation 8374] Now add the admin_view and its text to the options array
	...
	protected function getTheSite_viewSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [Interpretation 8374] Now add the site_view and its text to the options array
