...
class SermondistributorViewSermons extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		// [Interpretation 8728] Category Filter.
		JHtmlSidebar::addFilter(
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			// [Interpretation 8738] Category Batch selection.
			JHtmlBatch_::addListSelection(
		...
		if ($this->preacherNameOptions)
		{
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8656] Preacher Name Batch Selection
				JHtmlBatch_::addListSelection(
		...
		if ($this->seriesNameOptions)
		{
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8656] Series Name Batch Selection
				JHtmlBatch_::addListSelection(
		...
		// [Interpretation 8677] Set Link Type Selection
		$this->link_typeOptions = $this->getTheLink_typeSelections();
		if ($this->link_typeOptions)
		{
			// [Interpretation 8681] Link Type Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8690] Link Type Batch Selection
				JHtmlBatch_::addListSelection(
		...
		// [Interpretation 8677] Set Source Selection
		$this->sourceOptions = $this->getTheSourceSelections();
		if ($this->sourceOptions)
		{
			// [Interpretation 8681] Source Filter
			JHtmlSidebar::addFilter(
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 8690] Source Batch Selection
				JHtmlBatch_::addListSelection(
	...
	{
		// [Interpretation 8553] Get a db connection.
		$db = JFactory::getDbo();

		// [Interpretation 8555] Create a new query object.
		...
		// [Interpretation 8561] Reset the query using our newly populated query object.
		$db->setQuery($query);
		...
		if ($results)
		{
			...
			{
				// [Interpretation 8580] Translate the link_type selection
				$text = $model->selectionTranslation($link_type,'link_type');
				// [Interpretation 8582] Now add the link_type and its text to the options array
	...
	protected function getTheSourceSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [Interpretation 8580] Translate the source selection
				$text = $model->selectionTranslation($source,'source');
				// [Interpretation 8582] Now add the source and its text to the options array
