...
/------------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		ExternalsourcesfilterupdatemethodField.php
 ...
 * @since  1.6
 */
class ExternalsourcesfilterupdatemethodField extends ListField
{
	/**
	...
	protected function getOptions()
	{
		...
		$_filter = [];
		$_filter[] = Html::_('select.option', '', '- ' . Text::_('COM_SERMONDISTRIBUTOR_FILTER_SELECT_UPDATE_METHOD') . ' -');
		...
		if ($_results)
		{
			...
			foreach ($_results as $update_method)
			{
				// Translate the update_method selection
				$_text = $_model->selectionTranslation($update_method,'update_method');
				// Now add the update_method and its text to the options array
				$_filter[] = Html::_('select.option', $update_method, Text::_($_text));
			}
		}
		return $_filter;
	}
