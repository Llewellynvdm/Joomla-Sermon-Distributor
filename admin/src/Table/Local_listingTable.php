...
/------------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		Local_listingTable.php
...
/**
 * Local_listings Table class
 *
 * @since  1.6
 */
class Local_listingTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_sermondistributor.local_listing';
		...
		parent::__construct('#__sermondistributor_local_listing', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Local_listing table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New local_listing. A local_listing created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERMONDISTRIBUTOR_LOCAL_LISTING_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERMONDISTRIBUTOR_LOCAL_LISTING_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_sermondistributor.local_listing.'.$this->id);
			$this->setRules($rules);
		}
