 ...
 * @since  1.6
 */
class StatisticTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_sermondistributor.statistic';
		...
		parent::__construct('#__sermondistributor_statistic', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Statistic table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New statistic. A statistic created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERMONDISTRIBUTOR_STATISTIC_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERMONDISTRIBUTOR_STATISTIC_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_sermondistributor.statistic.'.$this->id);
			$this->setRules($rules);
		}
