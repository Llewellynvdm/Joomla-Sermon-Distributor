...
/------------------------------------------------------------------------------------------------------------------------------------/
	...
	@subpackage		External_sourceTable.php
...
/----------------------------------------------------------------------------------------------------------------------------------*/
namespace TrueChristianChurch\Component\Sermondistributor\Administrator\Table;
...
/**
 * External_sources Table class
 *
 * @since  1.6
 */
class External_sourceTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_sermondistributor.external_source';
		...
		parent::__construct('#__sermondistributor_external_source', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the External_source table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New external_source. A external_source created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERMONDISTRIBUTOR_EXTERNAL_SOURCE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERMONDISTRIBUTOR_EXTERNAL_SOURCE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_sermondistributor.external_source.'.$this->id);
			$this->setRules($rules);
		}
	...
	protected function _getAssetParentId(Table $table = null, $id = null)
	{
		...
		// load the sermondistributor asset
		$assets->loadByName('com_sermondistributor');
