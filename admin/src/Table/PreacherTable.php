 ...
 * @since  1.6
 */
class PreacherTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_sermondistributor.preacher';
		...
		parent::__construct('#__sermondistributor_preacher', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Preacher table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New preacher. A preacher created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERMONDISTRIBUTOR_PREACHER_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERMONDISTRIBUTOR_PREACHER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_sermondistributor.preacher.'.$this->id);
			$this->setRules($rules);
		}
