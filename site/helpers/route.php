...
abstract class SermondistributorHelperRoute
{
	...
	*/
	public static function getSermonRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// [Interpretation 3805] Initialize the needel array.
			$needles = array(
				'sermon'  => array((int) $id)
			);
			// [Interpretation 3809] Create the link
			$link = 'index.php?option=com_sermondistributor&view=sermon&id='. $id;
		}
		else
		{
			// [Interpretation 3814] Initialize the needel array.
			$needles = array();
			// [Interpretation 3816]Create the link but don't add the id.
