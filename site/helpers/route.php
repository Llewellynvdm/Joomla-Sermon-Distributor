...
abstract class SermondistributorHelperRoute
{
	...
	*/
	public static function getSermonRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// [Interpretation 3939] Initialize the needel array.
			$needles = array(
				'sermon'  => array((int) $id)
			);
			// [Interpretation 3943] Create the link
			$link = 'index.php?option=com_sermondistributor&view=sermon&id='. $id;
		}
		else
		{
			// [Interpretation 3948] Initialize the needel array.
			$needles = array();
			// [Interpretation 3950]Create the link but don't add the id.
