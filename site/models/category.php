...
class SermondistributorModelCategory extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// [2128] Get from #__sermondistributor_series as d
		$query->select($db->quoteName(
	...
	*/
	public function getCategory()
	{
		...
		// [2128] Get from #__categories as a
		$query->select($db->quoteName(
		...
		// [2378] set idCatidSermonB to the $data object.
	...
	*/
	public function getIdCatidSermonFbdf_B($id)
	{
		...
		// [3062] Get from #__sermondistributor_sermon as b
		$query->select($db->quoteName(
			array('b.id'),
			array('id')));
		...
		if ($db->getNumRows())
		{
			...
			// [3172] Convert the parameter fields into objects.
			foreach ($items as $nr => &$item)
			{
				// [2366] set idSermonStatisticC to the $item object.
	...
	*/
	public function getIdSermonStatisticFbdf_C($id)
	{
		...
		// [3062] Get from #__sermondistributor_statistic as c
		$query->select($db->quoteName(
			array('c.counter'),
			array('counter')));
