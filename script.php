...
class com_sermondistributorInstallerScript
{
	...
	{
		// [Interpretation 3430] Get Application object
		$app = JFactory::getApplication();

		// [Interpretation 3432] Get The Database object
		$db = JFactory::getDbo();

		// [Interpretation 3441] Create a new query object.
		$query = $db->getQuery(true);
		// [Interpretation 3443] Select id from content type table
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Preacher alias is found
		$query->where( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
		$db->setQuery($query);
		// [Interpretation 3449] Execute query to see if alias is found
		$db->execute();
		$preacher_found = $db->getNumRows();
		// [Interpretation 3452] Now check if there were any rows
		if ($preacher_found)
		{
			// [Interpretation 3455] Since there are load the needed  preacher type ids
			$preacher_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Preacher from the content type table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			// [Interpretation 3461] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__content_types'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [Interpretation 3470] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [Interpretation 3476] Remove Preacher items from the contentitem tag map table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [Interpretation 3487] If succesfully remove Preacher add queued success message.
			...
			// [Interpretation 3493] Remove Preacher items from the ucm content table
			$preacher_condition = array( $db->quoteName('core_type_alias') . ' = ' . $db->quote('com_sermondistributor.preacher') );
			// [Interpretation 3495] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__ucm_content'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [Interpretation 3504] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3510] Make sure that all the Preacher items are cleared from DB
			foreach ($preacher_ids as $preacher_id)
			{
				// [Interpretation 3515] Remove Preacher items from the ucm base table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [Interpretation 3517] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_base'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Preacher items
				$db->execute();

				// [Interpretation 3526] Remove Preacher items from the ucm history table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Preacher items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Sermon alias is found
		...
		if ($sermon_found)
		{
			// [Interpretation 3455] Since there are load the needed  sermon type ids
			$sermon_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Sermon from the content type table
			...
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [Interpretation 3470] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [Interpretation 3476] Remove Sermon items from the contentitem tag map table
			$sermon_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermon') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [Interpretation 3487] If succesfully remove Sermon add queued success message.
			...
			// [Interpretation 3493] Remove Sermon items from the ucm content table
			...
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [Interpretation 3504] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3510] Make sure that all the Sermon items are cleared from DB
			foreach ($sermon_ids as $sermon_id)
			{
				// [Interpretation 3515] Remove Sermon items from the ucm base table
				...
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Sermon items
				$db->execute();

				// [Interpretation 3526] Remove Sermon items from the ucm history table
				$sermon_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Sermon items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Sermon catid alias is found
		...
		{
			// [Interpretation 3455] Since there are load the needed  sermon_catid type ids
			$sermon_catid_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Sermon catid from the content type table
			...
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [Interpretation 3470] If succesfully remove Sermon catid add queued success message.
			...
			// [Interpretation 3476] Remove Sermon catid items from the contentitem tag map table
			$sermon_catid_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermons.category') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [Interpretation 3487] If succesfully remove Sermon catid add queued success message.
			...
			// [Interpretation 3493] Remove Sermon catid items from the ucm content table
			...
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [Interpretation 3504] If succesfully remove Sermon catid add queued success message.
			...
			// [Interpretation 3510] Make sure that all the Sermon catid items are cleared from DB
			foreach ($sermon_catid_ids as $sermon_catid_id)
			{
				// [Interpretation 3515] Remove Sermon catid items from the ucm base table
				...
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Sermon catid items
				$db->execute();

				// [Interpretation 3526] Remove Sermon catid items from the ucm history table
				$sermon_catid_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_catid_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Sermon catid items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Series alias is found
		...
		if ($series_found)
		{
			// [Interpretation 3455] Since there are load the needed  series type ids
			$series_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Series from the content type table
			...
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [Interpretation 3470] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [Interpretation 3476] Remove Series items from the contentitem tag map table
			$series_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.series') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($series_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [Interpretation 3487] If succesfully remove Series add queued success message.
			...
			// [Interpretation 3493] Remove Series items from the ucm content table
			...
			$query->where($series_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [Interpretation 3504] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3510] Make sure that all the Series items are cleared from DB
			foreach ($series_ids as $series_id)
			{
				// [Interpretation 3515] Remove Series items from the ucm base table
				...
				$query->where($series_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Series items
				$db->execute();

				// [Interpretation 3526] Remove Series items from the ucm history table
				$series_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $series_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($series_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Series items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Statistic alias is found
		...
		if ($statistic_found)
		{
			// [Interpretation 3455] Since there are load the needed  statistic type ids
			$statistic_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Statistic from the content type table
			...
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [Interpretation 3470] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [Interpretation 3476] Remove Statistic items from the contentitem tag map table
			$statistic_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.statistic') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [Interpretation 3487] If succesfully remove Statistic add queued success message.
			...
			// [Interpretation 3493] Remove Statistic items from the ucm content table
			...
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [Interpretation 3504] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3510] Make sure that all the Statistic items are cleared from DB
			foreach ($statistic_ids as $statistic_id)
			{
				// [Interpretation 3515] Remove Statistic items from the ucm base table
				...
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Statistic items
				$db->execute();

				// [Interpretation 3526] Remove Statistic items from the ucm history table
				$statistic_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $statistic_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Statistic items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3446] Where Help_document alias is found
		...
		{
			// [Interpretation 3455] Since there are load the needed  help_document type ids
			$help_document_ids = $db->loadColumn();
			// [Interpretation 3459] Remove Help_document from the content type table
			...
			$db->setQuery($query);
			// [Interpretation 3466] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [Interpretation 3470] If succesfully remove Help_document add queued success message.
			...
			// [Interpretation 3476] Remove Help_document items from the contentitem tag map table
			$help_document_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.help_document') );
			// [Interpretation 3478] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [Interpretation 3483] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [Interpretation 3487] If succesfully remove Help_document add queued success message.
			...
			// [Interpretation 3493] Remove Help_document items from the ucm content table
			...
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [Interpretation 3500] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [Interpretation 3504] If succesfully remove Help_document add queued success message.
			...
			// [Interpretation 3510] Make sure that all the Help_document items are cleared from DB
			foreach ($help_document_ids as $help_document_id)
			{
				// [Interpretation 3515] Remove Help_document items from the ucm base table
				...
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [Interpretation 3522] Execute the query to remove Help_document items
				$db->execute();

				// [Interpretation 3526] Remove Help_document items from the ucm history table
				$help_document_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $help_document_id);
				// [Interpretation 3528] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [Interpretation 3533] Execute the query to remove Help_document items
				$db->execute();
			}
		}

		// [Interpretation 3541] If All related items was removed queued success message.
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_base</b> table'));
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_history</b> table'));

		// [Interpretation 3546] Remove sermondistributor assets from the assets table
		$sermondistributor_condition = array( $db->quoteName('name') . ' LIKE ' . $db->quote('com_sermondistributor%') );

		// [Interpretation 3548] Create a new query object.
		$query = $db->getQuery(true);
		...
		if ($help_document_done);
		{
			// [Interpretation 3556] If succesfully remove sermondistributor add queued success message.
	...
	function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			// [Interpretation 3606] Get The Database object
			$db = JFactory::getDbo();

			// [Interpretation 3613] Create the preacher content type object.
			...
			// [Interpretation 3631] Set the object into the content types table.
			$preacher_Inserted = $db->insertObject('#__content_types', $preacher);

			// [Interpretation 3613] Create the sermon content type object.
			$sermon = new stdClass();
			...
			// [Interpretation 3613] Create the sermon catagory content type object.
			...
			// [Interpretation 3613] Create the series content type object.
			$series = new stdClass();
			...
			// [Interpretation 3613] Create the statistic content type object.
			...
			// [Interpretation 3613] Create the help_document content type object.
			...
			// [Interpretation 3667] Install the global extenstion params.
			$query = $db->getQuery(true);

			// [Interpretation 3675] Field to update.
			$fields = array(
		...
		if ($type == 'update')
		{
			...
			// [Interpretation 3622] Check if preacher type is already in content_type DB.
			$preacher_id = null;
			$query = $db->getQuery(true);
			...
			// [Interpretation 3622] Check if sermon type is already in content_type DB.
			$sermon_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3622] Check if sermon catagory type is already in content_type DB.
			...
			// [Interpretation 3622] Check if series type is already in content_type DB.
			$series_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3622] Check if statistic type is already in content_type DB.
			$statistic_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3622] Check if help_document type is already in content_type DB.
