...
class com_sermondistributorInstallerScript
{
	...
	{
		// [Interpretation 3432] Get Application object
		$app = JFactory::getApplication();

		// [Interpretation 3434] Get The Database object
		$db = JFactory::getDbo();

		// [Interpretation 3443] Create a new query object.
		$query = $db->getQuery(true);
		// [Interpretation 3445] Select id from content type table
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Preacher alias is found
		$query->where( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
		$db->setQuery($query);
		// [Interpretation 3451] Execute query to see if alias is found
		$db->execute();
		$preacher_found = $db->getNumRows();
		// [Interpretation 3454] Now check if there were any rows
		if ($preacher_found)
		{
			// [Interpretation 3457] Since there are load the needed  preacher type ids
			$preacher_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Preacher from the content type table
			...
			// [Interpretation 3478] Remove Preacher items from the contentitem tag map table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [Interpretation 3489] If succesfully remove Preacher add queued success message.
			...
			// [Interpretation 3495] Remove Preacher items from the ucm content table
			$preacher_condition = array( $db->quoteName('core_type_alias') . ' = ' . $db->quote('com_sermondistributor.preacher') );
			// [Interpretation 3497] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__ucm_content'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [Interpretation 3506] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3512] Make sure that all the Preacher items are cleared from DB
			foreach ($preacher_ids as $preacher_id)
			{
				// [Interpretation 3517] Remove Preacher items from the ucm base table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [Interpretation 3519] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_base'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Preacher items
				$db->execute();

				// [Interpretation 3528] Remove Preacher items from the ucm history table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Preacher items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Sermon alias is found
		...
		if ($sermon_found)
		{
			// [Interpretation 3457] Since there are load the needed  sermon type ids
			$sermon_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Sermon from the content type table
			...
			// [Interpretation 3478] Remove Sermon items from the contentitem tag map table
			$sermon_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermon') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [Interpretation 3489] If succesfully remove Sermon add queued success message.
			...
			// [Interpretation 3495] Remove Sermon items from the ucm content table
			...
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [Interpretation 3506] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3512] Make sure that all the Sermon items are cleared from DB
			foreach ($sermon_ids as $sermon_id)
			{
				// [Interpretation 3517] Remove Sermon items from the ucm base table
				...
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Sermon items
				$db->execute();

				// [Interpretation 3528] Remove Sermon items from the ucm history table
				$sermon_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Sermon items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Sermon catid alias is found
		...
		{
			// [Interpretation 3457] Since there are load the needed  sermon_catid type ids
			$sermon_catid_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Sermon catid from the content type table
			...
			// [Interpretation 3478] Remove Sermon catid items from the contentitem tag map table
			$sermon_catid_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermons.category') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [Interpretation 3489] If succesfully remove Sermon catid add queued success message.
			...
			// [Interpretation 3495] Remove Sermon catid items from the ucm content table
			...
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [Interpretation 3506] If succesfully remove Sermon catid add queued success message.
			...
			// [Interpretation 3512] Make sure that all the Sermon catid items are cleared from DB
			foreach ($sermon_catid_ids as $sermon_catid_id)
			{
				// [Interpretation 3517] Remove Sermon catid items from the ucm base table
				...
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Sermon catid items
				$db->execute();

				// [Interpretation 3528] Remove Sermon catid items from the ucm history table
				$sermon_catid_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_catid_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Sermon catid items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Series alias is found
		...
		if ($series_found)
		{
			// [Interpretation 3457] Since there are load the needed  series type ids
			$series_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Series from the content type table
			...
			// [Interpretation 3478] Remove Series items from the contentitem tag map table
			$series_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.series') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($series_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [Interpretation 3489] If succesfully remove Series add queued success message.
			...
			// [Interpretation 3495] Remove Series items from the ucm content table
			...
			$query->where($series_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [Interpretation 3506] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3512] Make sure that all the Series items are cleared from DB
			foreach ($series_ids as $series_id)
			{
				// [Interpretation 3517] Remove Series items from the ucm base table
				...
				$query->where($series_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Series items
				$db->execute();

				// [Interpretation 3528] Remove Series items from the ucm history table
				$series_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $series_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($series_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Series items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Statistic alias is found
		...
		if ($statistic_found)
		{
			// [Interpretation 3457] Since there are load the needed  statistic type ids
			$statistic_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Statistic from the content type table
			...
			// [Interpretation 3478] Remove Statistic items from the contentitem tag map table
			$statistic_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.statistic') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [Interpretation 3489] If succesfully remove Statistic add queued success message.
			...
			// [Interpretation 3495] Remove Statistic items from the ucm content table
			...
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [Interpretation 3506] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [Interpretation 3512] Make sure that all the Statistic items are cleared from DB
			foreach ($statistic_ids as $statistic_id)
			{
				// [Interpretation 3517] Remove Statistic items from the ucm base table
				...
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Statistic items
				$db->execute();

				// [Interpretation 3528] Remove Statistic items from the ucm history table
				$statistic_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $statistic_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Statistic items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [Interpretation 3448] Where Help_document alias is found
		...
		{
			// [Interpretation 3457] Since there are load the needed  help_document type ids
			$help_document_ids = $db->loadColumn();
			// [Interpretation 3461] Remove Help_document from the content type table
			...
			// [Interpretation 3478] Remove Help_document items from the contentitem tag map table
			$help_document_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.help_document') );
			// [Interpretation 3480] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [Interpretation 3485] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [Interpretation 3489] If succesfully remove Help_document add queued success message.
			...
			// [Interpretation 3495] Remove Help_document items from the ucm content table
			...
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [Interpretation 3502] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [Interpretation 3506] If succesfully remove Help_document add queued success message.
			...
			// [Interpretation 3512] Make sure that all the Help_document items are cleared from DB
			foreach ($help_document_ids as $help_document_id)
			{
				// [Interpretation 3517] Remove Help_document items from the ucm base table
				...
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [Interpretation 3524] Execute the query to remove Help_document items
				$db->execute();

				// [Interpretation 3528] Remove Help_document items from the ucm history table
				$help_document_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $help_document_id);
				// [Interpretation 3530] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [Interpretation 3535] Execute the query to remove Help_document items
				$db->execute();
			}
		}

		// [Interpretation 3543] If All related items was removed queued success message.
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_base</b> table'));
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_history</b> table'));

		// [Interpretation 3548] Remove sermondistributor assets from the assets table
		$sermondistributor_condition = array( $db->quoteName('name') . ' LIKE ' . $db->quote('com_sermondistributor%') );

		// [Interpretation 3550] Create a new query object.
		$query = $db->getQuery(true);
		...
		if ($help_document_done);
		{
			// [Interpretation 3558] If succesfully remove sermondistributor add queued success message.
	...
	function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			// [Interpretation 3608] Get The Database object
			$db = JFactory::getDbo();

			// [Interpretation 3615] Create the preacher content type object.
			...
			// [Interpretation 3633] Set the object into the content types table.
			$preacher_Inserted = $db->insertObject('#__content_types', $preacher);

			// [Interpretation 3615] Create the sermon content type object.
			$sermon = new stdClass();
			...
			// [Interpretation 3615] Create the sermon catagory content type object.
			...
			// [Interpretation 3615] Create the series content type object.
			$series = new stdClass();
			...
			// [Interpretation 3615] Create the statistic content type object.
			...
			// [Interpretation 3615] Create the help_document content type object.
			...
			// [Interpretation 3666] Install the global extenstion params.
			$query = $db->getQuery(true);

			// [Interpretation 3674] Field to update.
			$fields = array(
		...
		if ($type == 'update')
		{
			...
			// [Interpretation 3624] Check if preacher type is already in content_type DB.
			$preacher_id = null;
			$query = $db->getQuery(true);
			...
			// [Interpretation 3624] Check if sermon type is already in content_type DB.
			$sermon_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3624] Check if sermon catagory type is already in content_type DB.
			...
			// [Interpretation 3624] Check if series type is already in content_type DB.
			$series_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3624] Check if statistic type is already in content_type DB.
			$statistic_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [Interpretation 3624] Check if help_document type is already in content_type DB.
