...
class com_sermondistributorInstallerScript
{
	...
	function uninstall($parent)
	{
		...
		// [4529] Where Preacher alias is found
		...
		if ($preacher_found)
		{
			// [4538] Since there are load the needed  preacher type ids
			$preacher_ids = $db->loadColumn();
			// [4542] Remove Preacher from the content type table
			...
			$db->setQuery($query);
			// [4549] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4553] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4559] Remove Preacher items from the contentitem tag map table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			...
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [4566] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4570] If succesfully remove Preacher add queued success message.
			...
			// [4576] Remove Preacher items from the ucm content table
			...
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [4583] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4587] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4593] Make sure that all the Preacher items are cleared from DB
			foreach ($preacher_ids as $preacher_id)
			{
				// [4598] Remove Preacher items from the ucm base table
				...
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [4605] Execute the query to remove Preacher items
				$db->execute();

				// [4609] Remove Preacher items from the ucm history table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				...
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [4616] Execute the query to remove Preacher items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4529] Where Sermon alias is found
		...
		if ($sermon_found)
		{
			// [4538] Since there are load the needed  sermon type ids
			$sermon_ids = $db->loadColumn();
			// [4542] Remove Sermon from the content type table
			...
			$db->setQuery($query);
			// [4549] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4553] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4559] Remove Sermon items from the contentitem tag map table
			$sermon_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermon') );
			// [4561] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [4566] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4570] If succesfully remove Sermon add queued success message.
			...
			// [4576] Remove Sermon items from the ucm content table
			...
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [4583] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4587] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4593] Make sure that all the Sermon items are cleared from DB
			foreach ($sermon_ids as $sermon_id)
			{
				// [4598] Remove Sermon items from the ucm base table
				...
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [4605] Execute the query to remove Sermon items
				$db->execute();

				// [4609] Remove Sermon items from the ucm history table
				$sermon_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_id);
				// [4611] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [4616] Execute the query to remove Sermon items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4529] Where Sermon catid alias is found
		...
		{
			// [4538] Since there are load the needed  sermon_catid type ids
			$sermon_catid_ids = $db->loadColumn();
			// [4542] Remove Sermon catid from the content type table
			...
			$db->setQuery($query);
			// [4549] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4553] If succesfully remove Sermon catid add queued success message.
			...
			// [4559] Remove Sermon catid items from the contentitem tag map table
			$sermon_catid_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermons.category') );
			// [4561] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [4566] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4570] If succesfully remove Sermon catid add queued success message.
			...
			// [4576] Remove Sermon catid items from the ucm content table
			...
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [4583] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4587] If succesfully remove Sermon catid add queued success message.
			...
			// [4593] Make sure that all the Sermon catid items are cleared from DB
			foreach ($sermon_catid_ids as $sermon_catid_id)
			{
				// [4598] Remove Sermon catid items from the ucm base table
				...
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [4605] Execute the query to remove Sermon catid items
				$db->execute();

				// [4609] Remove Sermon catid items from the ucm history table
				$sermon_catid_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_catid_id);
				// [4611] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [4616] Execute the query to remove Sermon catid items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4529] Where Series alias is found
		...
		if ($series_found)
		{
			// [4538] Since there are load the needed  series type ids
			$series_ids = $db->loadColumn();
			// [4542] Remove Series from the content type table
			...
			$db->setQuery($query);
			// [4549] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4553] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4559] Remove Series items from the contentitem tag map table
			$series_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.series') );
			// [4561] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($series_condition);
			$db->setQuery($query);
			// [4566] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4570] If succesfully remove Series add queued success message.
			...
			// [4576] Remove Series items from the ucm content table
			...
			$query->where($series_condition);
			$db->setQuery($query);
			// [4583] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4587] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4593] Make sure that all the Series items are cleared from DB
			foreach ($series_ids as $series_id)
			{
				// [4598] Remove Series items from the ucm base table
				...
				$query->where($series_condition);
				$db->setQuery($query);
				// [4605] Execute the query to remove Series items
				$db->execute();

				// [4609] Remove Series items from the ucm history table
				$series_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $series_id);
				// [4611] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($series_condition);
				$db->setQuery($query);
				// [4616] Execute the query to remove Series items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4529] Where Statistic alias is found
		...
		if ($statistic_found)
		{
			// [4538] Since there are load the needed  statistic type ids
			$statistic_ids = $db->loadColumn();
			// [4542] Remove Statistic from the content type table
			...
			$db->setQuery($query);
			// [4549] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4553] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4559] Remove Statistic items from the contentitem tag map table
			$statistic_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.statistic') );
			// [4561] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [4566] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4570] If succesfully remove Statistic add queued success message.
			...
			// [4576] Remove Statistic items from the ucm content table
			...
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [4583] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4587] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4593] Make sure that all the Statistic items are cleared from DB
			foreach ($statistic_ids as $statistic_id)
			{
				// [4598] Remove Statistic items from the ucm base table
				...
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [4605] Execute the query to remove Statistic items
				$db->execute();

				// [4609] Remove Statistic items from the ucm history table
				$statistic_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $statistic_id);
				// [4611] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [4616] Execute the query to remove Statistic items
				$db->execute();
			}
		}
		...
		// [4629] Remove sermondistributor assets from the assets table
		...
		if ($help_document_done);
		{
			// [4639] If succesfully remove sermondistributor add queued success message.
	...
	function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			// [4692] Create the preacher content type object.
			...
			// [4692] Create the sermon content type object.
			$sermon = new stdClass();
			...
			// [4692] Create the sermon catagory content type object.
			...
			// [4692] Create the series content type object.
			$series = new stdClass();
			...
			// [4692] Create the statistic content type object.
