...
class com_sermondistributorInstallerScript
{
	...
	{
		// [4802] Get Application object
		$app = JFactory::getApplication();

		// [4804] Get The Database object
		$db = JFactory::getDbo();

		// [4813] Create a new query object.
		$query = $db->getQuery(true);
		// [4815] Select id from content type table
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Preacher alias is found
		$query->where( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
		$db->setQuery($query);
		// [4821] Execute query to see if alias is found
		$db->execute();
		$preacher_found = $db->getNumRows();
		// [4824] Now check if there were any rows
		if ($preacher_found)
		{
			// [4827] Since there are load the needed  preacher type ids
			$preacher_ids = $db->loadColumn();
			// [4831] Remove Preacher from the content type table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			// [4833] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__content_types'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [4838] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4842] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4848] Remove Preacher items from the contentitem tag map table
			$preacher_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.preacher') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4859] If succesfully remove Preacher add queued success message.
			...
			// [4865] Remove Preacher items from the ucm content table
			$preacher_condition = array( $db->quoteName('core_type_alias') . ' = ' . $db->quote('com_sermondistributor.preacher') );
			// [4867] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__ucm_content'));
			$query->where($preacher_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Preacher items
			$preacher_done = $db->execute();
			if ($preacher_done);
			{
				// [4876] If succesfully remove Preacher add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.preacher) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4882] Make sure that all the Preacher items are cleared from DB
			foreach ($preacher_ids as $preacher_id)
			{
				// [4887] Remove Preacher items from the ucm base table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [4889] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_base'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Preacher items
				$db->execute();

				// [4898] Remove Preacher items from the ucm history table
				$preacher_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $preacher_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($preacher_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Preacher items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Sermon alias is found
		...
		if ($sermon_found)
		{
			// [4827] Since there are load the needed  sermon type ids
			$sermon_ids = $db->loadColumn();
			// [4831] Remove Sermon from the content type table
			...
			$db->setQuery($query);
			// [4838] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4842] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4848] Remove Sermon items from the contentitem tag map table
			$sermon_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermon') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4859] If succesfully remove Sermon add queued success message.
			...
			// [4865] Remove Sermon items from the ucm content table
			...
			$query->where($sermon_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Sermon items
			$sermon_done = $db->execute();
			if ($sermon_done);
			{
				// [4876] If succesfully remove Sermon add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.sermon) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4882] Make sure that all the Sermon items are cleared from DB
			foreach ($sermon_ids as $sermon_id)
			{
				// [4887] Remove Sermon items from the ucm base table
				...
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Sermon items
				$db->execute();

				// [4898] Remove Sermon items from the ucm history table
				$sermon_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Sermon items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Sermon catid alias is found
		...
		{
			// [4827] Since there are load the needed  sermon_catid type ids
			$sermon_catid_ids = $db->loadColumn();
			// [4831] Remove Sermon catid from the content type table
			...
			$db->setQuery($query);
			// [4838] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4842] If succesfully remove Sermon catid add queued success message.
			...
			// [4848] Remove Sermon catid items from the contentitem tag map table
			$sermon_catid_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.sermons.category') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4859] If succesfully remove Sermon catid add queued success message.
			...
			// [4865] Remove Sermon catid items from the ucm content table
			...
			$query->where($sermon_catid_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Sermon catid items
			$sermon_catid_done = $db->execute();
			if ($sermon_catid_done);
			{
				// [4876] If succesfully remove Sermon catid add queued success message.
			...
			// [4882] Make sure that all the Sermon catid items are cleared from DB
			foreach ($sermon_catid_ids as $sermon_catid_id)
			{
				// [4887] Remove Sermon catid items from the ucm base table
				...
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Sermon catid items
				$db->execute();

				// [4898] Remove Sermon catid items from the ucm history table
				$sermon_catid_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $sermon_catid_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($sermon_catid_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Sermon catid items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Series alias is found
		...
		if ($series_found)
		{
			// [4827] Since there are load the needed  series type ids
			$series_ids = $db->loadColumn();
			// [4831] Remove Series from the content type table
			...
			$db->setQuery($query);
			// [4838] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4842] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4848] Remove Series items from the contentitem tag map table
			$series_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.series') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($series_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4859] If succesfully remove Series add queued success message.
			...
			// [4865] Remove Series items from the ucm content table
			...
			$query->where($series_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Series items
			$series_done = $db->execute();
			if ($series_done);
			{
				// [4876] If succesfully remove Series add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.series) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4882] Make sure that all the Series items are cleared from DB
			foreach ($series_ids as $series_id)
			{
				// [4887] Remove Series items from the ucm base table
				...
				$query->where($series_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Series items
				$db->execute();

				// [4898] Remove Series items from the ucm history table
				$series_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $series_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($series_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Series items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Statistic alias is found
		...
		if ($statistic_found)
		{
			// [4827] Since there are load the needed  statistic type ids
			$statistic_ids = $db->loadColumn();
			// [4831] Remove Statistic from the content type table
			...
			$db->setQuery($query);
			// [4838] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4842] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__content_type</b> table'));
			}

			// [4848] Remove Statistic items from the contentitem tag map table
			$statistic_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.statistic') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4859] If succesfully remove Statistic add queued success message.
			...
			// [4865] Remove Statistic items from the ucm content table
			...
			$query->where($statistic_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Statistic items
			$statistic_done = $db->execute();
			if ($statistic_done);
			{
				// [4876] If succesfully remove Statistic add queued success message.
				$app->enqueueMessage(JText::_('The (com_sermondistributor.statistic) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4882] Make sure that all the Statistic items are cleared from DB
			foreach ($statistic_ids as $statistic_id)
			{
				// [4887] Remove Statistic items from the ucm base table
				...
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Statistic items
				$db->execute();

				// [4898] Remove Statistic items from the ucm history table
				$statistic_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $statistic_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($statistic_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Statistic items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4818] Where Help_document alias is found
		...
		{
			// [4827] Since there are load the needed  help_document type ids
			$help_document_ids = $db->loadColumn();
			// [4831] Remove Help_document from the content type table
			...
			$db->setQuery($query);
			// [4838] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4842] If succesfully remove Help_document add queued success message.
			...
			// [4848] Remove Help_document items from the contentitem tag map table
			$help_document_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_sermondistributor.help_document') );
			// [4850] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [4855] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4859] If succesfully remove Help_document add queued success message.
			...
			// [4865] Remove Help_document items from the ucm content table
			...
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [4872] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4876] If succesfully remove Help_document add queued success message.
			...
			// [4882] Make sure that all the Help_document items are cleared from DB
			foreach ($help_document_ids as $help_document_id)
			{
				// [4887] Remove Help_document items from the ucm base table
				...
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [4894] Execute the query to remove Help_document items
				$db->execute();

				// [4898] Remove Help_document items from the ucm history table
				$help_document_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $help_document_id);
				// [4900] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [4905] Execute the query to remove Help_document items
				$db->execute();
			}
		}

		// [4913] If All related items was removed queued success message.
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_base</b> table'));
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_history</b> table'));

		// [4918] Remove sermondistributor assets from the assets table
		$sermondistributor_condition = array( $db->quoteName('name') . ' LIKE ' . $db->quote('com_sermondistributor%') );

		// [4920] Create a new query object.
		$query = $db->getQuery(true);
		...
		if ($help_document_done);
		{
			// [4928] If succesfully remove sermondistributor add queued success message.
	...
	function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			// [4978] Get The Database object
			$db = JFactory::getDbo();

			// [4985] Create the preacher content type object.
			...
			// [5003] Set the object into the content types table.
			$preacher_Inserted = $db->insertObject('#__content_types', $preacher);

			// [4985] Create the sermon content type object.
			$sermon = new stdClass();
			...
			// [4985] Create the sermon catagory content type object.
			...
			// [4985] Create the series content type object.
			$series = new stdClass();
			...
			// [4985] Create the statistic content type object.
			...
			// [4985] Create the help_document content type object.
			...
			// [5036] Install the global extenstion params.
			$query = $db->getQuery(true);

			// [5044] Field to update.
			$fields = array(
		...
		if ($type == 'update')
		{
			...
			// [4994] Check if preacher type is already in content_type DB.
			$preacher_id = null;
			$query = $db->getQuery(true);
			...
			// [4994] Check if sermon type is already in content_type DB.
			$sermon_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [4994] Check if sermon catagory type is already in content_type DB.
			...
			// [4994] Check if series type is already in content_type DB.
			$series_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [4994] Check if statistic type is already in content_type DB.
			$statistic_id = null;
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('type_id')));
			$query->from($db->quoteName('#__content_types'));
			...
			// [4994] Check if help_document type is already in content_type DB.
